...
import { MANAGER } from "@/lib/mock-manager-data"
...
const SIDEBAR_COLLAPSED_KEY = "manager_sidebar_collapsed"
const SIDEBAR_COLLAPSED_EVENT = "manager-sidebar-collapsed-change"
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 overflow-hidden whitespace-nowrap transition-all duration-250" style={{ opacity: expanded ? 1 : 0, maxWidth: expanded ? 130 : 0 }}>
        <p className="truncate text-[13px] font-bold leading-tight text-white">{MANAGER.name}</p>
        <p className="text-[11px]" style={{ color: "rgba(255,255,255,0.45)" }}>Gestor</p>
      </div>
    </div>
  )
}
...
export function ManagerChrome({ children }: { children: React.ReactNode }) {
